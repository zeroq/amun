...
class vuln:
	...
	def __init__(self):
		try:
			self.vuln_name = "MDAEMON Vulnerability"
			self.stage = "MDAEMON_STAGE1"
	...
	def incoming(self, message, bytes, ip, vuLogger, random_reply, ownIP):
		try:
			...
			if self.stage=="MDAEMON_STAGE1" and bytes>=1024 and (message.startswith('USER') or message.startswith('user')):
