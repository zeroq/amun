...
import amun_logging
import random
import bagle_shellcodes

class vuln:
	def __init__(self):
		try:
			self.vuln_name = "BAGLE Vulnerability"
			self.stage = "BAGLE_STAGE1"
	...
	def incoming(self, message, bytes, ip, vuLogger, random_reply, ownIP):
		try:
			### logging object
			self.log_obj = amun_logging.amun_logging("vuln_bagle", vuLogger)
			...
			if bytes>0:
				self.log_obj.log("incoming bagle: %s (length: %s)" % (message, len(message)), 9, "debug", True, True)

			if self.stage == "BAGLE_STAGE1" and bytes==39:
				if message == bagle_shellcodes.bagle_auth:
					resultSet["result"] = True
					resultSet["accept"] = True
					resultSet["reply"] = "12345678"
					self.stage = "BAGLE_STAGE2"
					return resultSet
			elif self.stage == "BAGLE_STAGE2":
				resultSet["result"] = True
				resultSet["accept"] = True
