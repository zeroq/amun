...
import struct
import random
import tivoli_shellcodes

import amun_logging

### IBM Tivoli Provisioning Manager PRE AUTH Remote Exploit
### Works for webdesproxy (cygwin) remote buffer overflow, as well

class vuln:
	def __init__(self):
		try:
			self.vuln_name = "TIVOLI Vulnerability"
			self.stage = "TIVOLI_STAGE1"
	...
	def incoming(self, message, bytes, ip, vuLogger, random_reply, ownIP):
		try:
			### logging obj
			self.log_obj = amun_logging.amun_logging("vuln_tivoli", vuLogger)
			...
			if self.stage=="TIVOLI_STAGE1" and bytes>=1024:
