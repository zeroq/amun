...
class vuln:
	...
	def __init__(self):
		try:
			self.vuln_name = "CYRUSPOP3 Vulnerability"
			self.stage = "CYRUSPOP3_STAGE1"
			self.welcome_message = "cyrus-imapd-2.3.2"
			self.shellcode = []
	...
	def incoming(self, message, bytes, ip, vuLogger, random_reply, ownIP):
		try:
			...
			if self.stage=="CYRUSPOP3_STAGE1" and bytes>1000:
				resultSet['result'] = True
				resultSet['accept'] = True
				resultSet['shutdown'] = True
				return resultSet
			elif self.stage=="CYRUSPOP3_STAGE1" and bytes==533:
				resultSet['result'] = False
				resultSet['accept'] = True
